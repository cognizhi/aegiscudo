...
The Python service now runs local npm, PyPI, Cargo, and Maven/JVM sandbox profiles with canary planting, timeout handling, stdout/stderr redaction, structured telemetry emission, and focused integration coverage. The Maven path currently provides an initial `jvm-binary-profile`: it selects classes from packaged `.jar` / `.war` / `.ear` artifacts, uses a temporary Java source probe plus `Class.forName(..., true, ...)` to trigger class loading and static initializers in-process, emits `jvm-class-loaded` telemetry for selected artifact classes confirmed in `java -verbose:class` output, polls `/proc` for descendant processes during class loading, and snapshots sandbox-owned home/workspace files before and after phase G so coarse `jvm-process-execution-observed` and `jvm-filesystem-write-observed` events are preserved alongside `outbound-network-attempt` and `canary-secret-access`. `outbound-network-attempt` telemetry carries normalized `destination_url`, `destination_host`, and `destination_ip` fields so downstream policy evaluation can correlate sandbox egress against feed-harvested cross-ecosystem IOC records. Higher-fidelity JVM runtime attribution, syscall and packet-level capture, and production worker deployment are governed by [High-Fidelity Detonation Worker](../high-fidelity-detonation.md).