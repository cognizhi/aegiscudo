# Vendored SBOM Schema Provenance

These files are vendored for deterministic offline SBOM export validation in `aedo-cli` tests.
Do not edit them by hand; refresh them from the upstream source and update this note when the upstream version changes.

Retrieved: 2026-05-11

## CycloneDX 1.7

- `schemas/external/cyclonedx/1.7/bom-1.7.schema.json`
  - Upstream: `https://cyclonedx.org/schema/bom-1.7.schema.json`
  - SHA-256: `71152f97948eeeca2fd4a1434a9d29aab35d377be11828b504d029dfeeb1925a`
- `schemas/external/cyclonedx/1.7/spdx.schema.json`
  - Upstream: `https://cyclonedx.org/schema/spdx.schema.json`
  - SHA-256: `c87aa7bb5eb503d40b52ec6bf00de8045df15da7a13cea48d290cf6d36a8d2ea`
- `schemas/external/cyclonedx/1.7/jsf-0.82.schema.json`
  - Upstream: `https://cyclonedx.org/schema/jsf-0.82.schema.json`
  - SHA-256: `2faf5eb3651f2ae5f46091a131770d8d847bbd121139d19c85fc7051bfa58c46`
- `schemas/external/cyclonedx/1.7/cryptography-defs.schema.json`
  - Upstream: `https://cyclonedx.org/schema/cryptography-defs.schema.json`
  - SHA-256: `95897594e42f7229523fd7a03272da284e0591b1c49550a8b62a355df6357249`

## CycloneDX 1.6

- `schemas/external/cyclonedx/1.6/bom-1.6.schema.json`
  - Upstream: `https://cyclonedx.org/schema/bom-1.6.schema.json`
  - SHA-256: `1ebcb88a2c845ecb6ff7bee7aeabdff9422cb0347f3d6875b241bd444b7e098f`
- `schemas/external/cyclonedx/1.6/spdx.schema.json`
  - Upstream: `https://cyclonedx.org/schema/spdx.schema.json`
  - SHA-256: `c87aa7bb5eb503d40b52ec6bf00de8045df15da7a13cea48d290cf6d36a8d2ea`
- `schemas/external/cyclonedx/1.6/jsf-0.82.schema.json`
  - Upstream: `https://cyclonedx.org/schema/jsf-0.82.schema.json`
  - SHA-256: `2faf5eb3651f2ae5f46091a131770d8d847bbd121139d19c85fc7051bfa58c46`

## SPDX 2.3

- `schemas/external/spdx/2.3/spdx-schema.json`
  - Upstream: `https://raw.githubusercontent.com/spdx/spdx-spec/v2.3/schemas/spdx-schema.json`
  - SHA-256: `239208b7ac287b3cf5d9a9af23f9d69863971102a5e1587a27a398b43490b89b`

## Tracked Future Compatibility Target

- SPDX 3.0 is the next SBOM compatibility target called out by the Phase 2 PRD.
- It is intentionally not vendored here yet because the current service and CLI only emit validated SPDX 2.3 JSON compatibility exports.
- When SPDX 3.0 export work starts, vendor the exact upstream validation assets here and update this provenance note alongside the new export path.