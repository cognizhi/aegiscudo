...
test.describe("Analysis: GitHub Actions Workflow Integrity", () => {
  test("renders scan result rows with owner/repo, ref, decision, and rationale", async ({
    ...
    await expect(
      page.getByTestId(`gha-row-${mockResults[0].id}`),
    ).toBeVisible();
    await expect(
      page.getByTestId(`gha-row-${mockResults[1].id}`),
    ).toBeVisible();
    ...
    const row1 = page.getByTestId(`gha-row-${mockResults[0].id}`);
    ...
    const row2 = page.getByTestId(`gha-row-${mockResults[1].id}`);
  ...
  test("shows BLOCK decision badge with correct visual class", async ({
    ...
    await page.route(
      `**/api/tenants/${tenantId}/github-actions/scan-results**`,
      async (route) => route.fulfill({ json: [mockResults[1]] }),
    );

    await openEvidence(page);

    const badge = page.locator(
      `[data-testid="gha-row-${mockResults[1].id}"] [data-decision="BLOCK_POLICY_VIOLATION"]`,
    );
