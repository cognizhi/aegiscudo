...
async fn main() -> anyhow::Result<()> {
    ...
    let database_url = std::env::var("DATABASE_URL")
        .context("DATABASE_URL is required for sbom-service")?;
    let pool = connect(&database_url)
        .await
        .context("connecting sbom-service database pool")?;
    let sbom_store_dir = std::env::var("SBOM_STORE_DIR")
        .unwrap_or_else(|_| "/var/lib/aegiscudo/sboms".to_owned());
    let config = Config { sbom_store_dir: sbom_store_dir.into() };
